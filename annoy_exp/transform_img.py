...
def transform_image(img_path, profile_id):
	...
	'''this for loop creates multple images'''
	pair_list = []
	for k in range(45):
		alpha_list = [1.0, 1.5, 2.0]  #values of alpha from 1.0 to 30, best result til 2.0.
		beta_list = [x for x in range(20, 40, 10)] #beta values from 1 to 100 best till 60

		#randomly selection alpha and beta from list
		...
		#checking that pair of alpha and beta already used or not
		if pair in pair_list:
			...
			#transforming image basic linear transform
			mul_img = cv2.multiply(img,np.array([alpha]))
			new_img = cv2.add(mul_img, beta)

			#con_img = convolve_image(new_img)
			#cv2.imshow(pair, con_img)
			#cv2.waitKey()
			#affine_transform(con_img, profile_path, name)

			#calculationg convolve of the image
			con_img = convolve_image(img)
			save_path = profile_path + '/' + str(profile_id) + '_' + str('convolve') + '.jpg'

			#save_path = profile_path + '/' + name + '.jpg'
			#cv2.imshow('test1', con_img)
			#cv2.waitKey()
...
if __name__ == '__main__':
	img_path = '../rahul_07.jpg'
	main_fun(img_path, 'test_rahul_test')
